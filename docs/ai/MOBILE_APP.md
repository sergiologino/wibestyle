...
Paywall показывает отдельный бесплатный trial на 3 примерки и одно успешное видео для авторизованного trial-профиля. Онбординг больше не открывает trial напрямую: финальный CTA и пропуск ведут на auth с `next=/paywall`, после регистрации/авторизации открывается paywall, затем пользователь выбирает trial или оплату. Платные квоты: Wibe — 20 в месяц или 240 в год; Elite — 100 в месяц или 1200 в год. Рекомендуется годовой Elite; его карточка выделяет видео к каждой примерке, лучшие нейросети и приоритетную поддержку. Годовые карточки показывают экономию относительно 12 месячных платежей, а активная скидка с лендинга помечается как уже включённая в цену.

RuStore review prompt есть только в Android-приложении. После успешных примерок локальный счетчик показывает satisfaction gate: trial-пользователям после 3 уникальных успешных сессий, платным Wibe/Elite — после 5. Позитивный ответ открывает просьбу оставить честный отзыв в RuStore, негативный — внутренний feedback с причинами (`bad_fit`, `wrong_item`, `slow_generation`, `photo_problem`, `other`). "Позже" ставит cooldown 30 дней, "Больше не показывать" отключает prompt локально. Бонусы за RuStore-отзыв, просьба о 5 звездах и проверка скриншотов намеренно не используются.
