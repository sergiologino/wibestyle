...
### Avatar enhancement — готово к deploy smoke test (2026-08-10)

- `noteapp-ai-integration`: новая сеть `wibestyle-avatar-enhance` (`gpt-image-1.5`, `image_edit`, `input_fidelity: high`) повторно использует зашифрованный ключ существующей сети `openai-gpt4o-mini`.
- API, web и Android предлагают улучшение только при неблокирующих предупреждениях о свете или фоне, хранят preview отдельно от исходника и требуют явного выбора перед применением.
- Проверено локально: API integration tests, web tests/build и mobile tests/type-check. Перед закрытием P0 нужен один production smoke test с реальным разрешённым фото и проверкой apply/revert.
