...
## npm install в Nixpacks/Coolify

- Текущий deployment image использует Node 22.11; версии в корневом `package-lock.json` закреплены с учетом этой версии.
- В production всегда используйте `npm ci`, а не `npm install`, чтобы сборка точно воспроизводила lockfile.
- Репозиторий содержит `.npmrc` с ограниченными повторами и увеличенными таймаутами на случай временного `ECONNRESET` от npm registry. Если все попытки исчерпаны, повторите deploy: это сетевая ошибка загрузки, а не ошибка компиляции.
- Не обновляйте отдельно React Native, Vite или jsdom без проверки требований Expo SDK и версии Node в Nixpacks.

### Восстановление поврежденного `node_modules`

Если локальный `npm install` завершается с `ENOENT` для отсутствующего optional-пакета (например, `@tailwindcss/oxide-wasm32-wasi`), выполните из корня проекта:

```powershell
npm run deps:repair
```

Команда безопасно удаляет только каталоги `node_modules` внутри monorepo и выполняет чистую установку точных версий из `package-lock.json` с platform-specific optional-зависимостями. После нее обычный `npm install` должен снова работать. Не исправляйте такой сбой ручным изменением lock-файла и не запускайте `npm audit fix --force`: это не устраняет повреждение дерева зависимостей и может несовместимо обновить Expo/React Native/Next.js.
...
| Backend deploy в Coolify/Nixpacks падает на этапе `#11 npm warn EBADENGINE` / `npm install` | API запущен из корня monorepo, и Nixpacks определил Node workspaces вместо Gradle backend. Для API выставьте **Base directory**: `services/api` (или `/services/api`, если Coolify требует leading slash), **Build command**: `chmod +x gradlew && ./gradlew bootJar -x test`, **Start command**: `java -jar build/libs/wibestyle-api.jar`, **Port**: `8080`. Backend deploy не должен выполнять `npm install`. |
| Backend deploy после перехода на `services/api` падает с `monorepoRoot must not be null` | Обновите API до версии, где `services/api/build.gradle.kts` поддерживает isolated build root `/app`. Настройки Coolify откатывать на корень monorepo не нужно. |
