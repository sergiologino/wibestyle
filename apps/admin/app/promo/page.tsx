...
function draftFromPromo(item: PromoCodeRecord): PromoDraft {
  return {
...
export default function AdminPromoPage() {
  ...
  function startEdit(item: PromoCodeRecord) {
    setEditingId(item.id);
    setEditDraft(draftFromPromo(item));
    setError(null);
    setMessage(null);
  }

  async function onSaveEdit(item: PromoCodeRecord) {
    if (!editDraft) return;
    setError(null);
    setMessage(null);
    try {
      await api.updateAdminPromoCode(adminKey, item.id, {
        discountPercent: editDraft.discountPercent,
        maxUses: editDraft.maxUses,
        expiresAt: new Date(editDraft.expiresAt).toISOString(),
        label: editDraft.label.trim() || undefined,
      });
      setEditingId(null);
      setEditDraft(null);
      setMessage(`Промокод ${item.code} обновлён`);
      await load(adminKey);
    } catch {
      setError("Не удалось сохранить промокод. Проверьте скидку, лимит и дату.");
    }
  }
        ...
        <form className="mt-4 grid gap-3 md:grid-cols-2" onSubmit={onCreate}>
          <AdminField label="Код промо" hint="Пусто — автогенерация" className="md:col-span-2">
            <div className="flex gap-2">
              ...
              />
              <Button type="button" variant="secondary" onClick={() => void onGenerateCode()}>
                Сгенерировать
              </Button>
            </div>
          </AdminField>
              ...
              <div key={item.id} className="rounded-[24px] border border-[#ffd1ed] p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-[240px] flex-1">
                    ...
                    {!editing ? (
                      <p className="font-bold text-[#6d6273]">
                        −{item.discountPercent}% · {item.usesCount}/{item.maxUses} · до {new Date(item.expiresAt).toLocaleString("ru-RU")}
                      ...
                      <div className="mt-3 grid gap-3 md:grid-cols-2">
                        <AdminField label="Скидка, %">
                          <input
                            ...
                            onChange={(event) => setEditDraft({ ...editDraft, discountPercent: Number(event.target.value) })}
                          />
                        </AdminField>
                        <AdminField label="Лимит регистраций" hint={`Не меньше уже использовано: ${item.usesCount}`}>
                          <input
                            className="rounded-2xl border border-[#ffd1ed] px-4 py-3 font-bold"
                            type="number"
                            min={Math.max(1, item.usesCount)}
                            value={editDraft.maxUses}
                            onChange={(event) => setEditDraft({ ...editDraft, maxUses: Number(event.target.value) })}
                          />
                        </AdminField>
                        <AdminField label="Действует до">
                          <input
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {editing ? (
                      <>
                        <Button size="md" onClick={() => void onSaveEdit(item)}>
                          Сохранить
                        </Button>
                        <Button
                          size="md"
                    ...
                    {item.active ? (
                      ...
                      <Pill tone="soft">Не активен</Pill>
                    )}
                  </div>
                </div>
              </div>
