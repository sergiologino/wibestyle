...
export default function PwaInstallPrompt() {
  ...
  return (
    <aside aria-label="Установка приложения" className="fixed inset-x-3 bottom-[max(0.75rem,env(safe-area-inset-bottom))] z-50 rounded-3xl border border-white/70 bg-[#302637]/95 p-4 text-white shadow-2xl backdrop-blur sm:left-auto sm:right-4 sm:w-[360px]">
      ...
      <p className="pr-7 text-base font-semibold">Установите «Я на стиле»</p>
      <p className="mt-1 text-sm leading-5 text-white/80">Ярлык появится на экране телефона, а примерочная будет открываться без панели браузера.</p>
      {manualInstructions ? <p className="mt-3 rounded-2xl bg-white/10 px-3 py-2 text-sm leading-5">{isIosDevice() ? "Откройте страницу в Safari, нажмите «Поделиться», затем выберите «На экран „Домой“»." : "Откройте меню браузера ⋮ и выберите «Установить приложение»."}</p> : null}
      <div className="mt-3 flex gap-2">
        <Button className="flex-1" size="sm" type="button" onClick={() => void install()}>{manualInstructions ? "Понятно" : "Установить"}</Button>
