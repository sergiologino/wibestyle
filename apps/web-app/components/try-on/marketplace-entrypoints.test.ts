...
describe("marketplace try-on entry points", () => {
  it("keeps a direct WB/Ozon action on the authenticated home and header", () => {
    const home = readFileSync(join(process.cwd(), "components", "home", "HomeDashboardClient.tsx"), "utf8");
    const header = readFileSync(join(process.cwd(), "components", "AppTopBar.tsx"), "utf8");

    expect(home).toContain('data-testid="marketplace-try-on-primary"');
    expect(home).toContain('href="/try-on/link"');
    expect(header).toContain('data-testid="marketplace-try-on-header"');
    expect(header).toContain('href="/try-on/link"');
  });
