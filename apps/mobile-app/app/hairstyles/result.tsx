...
export default function HairstyleResultScreen() {
  const router = useRouter(); const { styleId, imagePath } = useLocalSearchParams<{ styleId: string; imagePath: string }>();
  const { accessToken } = useSession(); const style = getHairstyle(styleId);
  if (!style || !imagePath) return null;
  return <Screen><ScrollView contentContainerStyle={sheet.scroll} showsVerticalScrollIndicator={false}>
    ...
    <Eyebrow>Готово</Eyebrow><DisplayTitle>{style.title}</DisplayTitle><BodyText>Это визуализация: финальную форму и уход лучше обсудить с мастером.</BodyText>
    <AuthenticatedImage path={imagePath} accessToken={accessToken} style={sheet.image} contentFit="cover" />
    <View style={sheet.note}><Text style={sheet.noteTitle}>Что сказать мастеру</Text><Text style={sheet.noteText}>{style.masterNote}</Text></View>
