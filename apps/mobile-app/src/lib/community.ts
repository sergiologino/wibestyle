...
export function getTelegramChannelUrl(): string | null {
  ...
  const raw = fromExtra ?? process.env.EXPO_PUBLIC_TELEGRAM_CHANNEL_URL ?? "https://t.me/vibestyle_channel";
...
export function getTelegramChannelName(): string {
  ...
  return (fromExtra ?? process.env.EXPO_PUBLIC_TELEGRAM_CHANNEL_NAME ?? "Я на стиле. Поддержка").trim() || "Я на стиле. Поддержка";
}
