...
describe("RuStore review prompt logic", () => {
  ...
  it("does not show during 14-day postpone or negative feedback cooldown", () => {
    const now = new Date("2026-07-30T10:00:00.000Z");
    ...
    expect(shouldShowReviewPromptGate(deferReviewPromptInState(ready, now), "trial", new Date("2026-08-13T09:59:59.000Z"))).toBe(false);
    expect(shouldShowReviewPromptGate(deferReviewPromptInState(ready, now), "trial", new Date("2026-08-13T10:00:01.000Z"))).toBe(true);
  });
});
